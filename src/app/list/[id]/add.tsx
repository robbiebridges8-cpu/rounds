...
import { useAddListPubs, useListPubs } from '@/lib/lists';
...
/** Add pubs to a list. Same sheet as the quest one, pointed at lists. */
export default function AddPubsSheet() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const router = useRouter();
  const existing = useListPubs(id);
  const add = useAddListPubs();
